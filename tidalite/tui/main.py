...
class TUIApp:
    ...
    async def _handle_input(self):
        ...
        if key == '\x1b':
            key_suffix = await get_key() + await get_key()
            key = {'[a': 'up', '[b': 'down', '[c': 'right', '[d': 'left'}.get(key_suffix.lower(), 'esc')

        if key == 'q': self.running = False
        elif key == ' ': self.player.toggle_pause()
        elif key in ('b', 'esc'): await self.pop_view()
        elif key == '\x7f': key = "backspace" # normalize backspace key
